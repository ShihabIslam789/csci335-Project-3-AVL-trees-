...
public:
    ...
    //  a float varibale gets incremented with every recursive call
    //  to the private funct find
    Comparable find(const Comparable& x, float& counter) const {
        return find(x, root, ++counter);
    }

    //  this function calls a private function which will return counter function
    int TotalNodeCount() {
        return NumofNodes(root);
    }


    // this function calls the private Depth function
    int Depth(AvlTree& node) {
        return DepthofNode(node, root);
    }

    //   public member function which  which  calculates the internal path length
    int InternalPathLength() {
        return InternalPathLength(root);
    }

    //  this function calls the internal path function, and the
    //  counter function and divide them to return a float
    float AverageDepth() {
        return InternalPathLength(root) / NumofNodes(root);
    }

    //  this function will call the member function that calculates
    // the average depth and divides it by the log of the number of node
    float LogRatio() {
...
private:
    ...
    //  a float variable which will be incremented everytime find is  recursively called
    ...
    *****************************************************/
    ...
    //  this function will traverse through a tree, and count the number of  nodes 
    ...
    //  this function finds the depth of a single node in the tree
    ...
    //  this function counts the internal path length of the whole tree
